...
> **Core interview habit:** do not jump from prompt to technology. Move through **requirements → scale → contracts/data → simple architecture → flows → deep dive → failures → trade-offs → summary**.

A strong L4 answer is usually a design you can explain and defend, not the architecture with the most components.

# Topics to Revise

- [ ] functional vs non-functional requirements
- [ ] non-goals / scope control
- [ ] decision-relevant estimation
- [ ] APIs/events before infrastructure details
- [ ] access patterns and data model
- [ ] simple HLD first
- [ ] read/write request-flow narration
